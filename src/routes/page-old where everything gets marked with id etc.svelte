        ...
        if (matchedPairs.length === gamePokemon.length) {
         ...
         // Create a new leaderboard entry
            const newEntry: LeaderboardEntry = {
                id: Date.now(), // Verwenden Sie den aktuellen Zeitstempel als eindeutige ID
                time: elapsedSeconds,
                moves: attempts,
                player: "Anonym"
            };
...
function updateLeaderboard() {
    if (playerName.trim()) {
        const updatedEntry = leaderboard.find(entry => entry.id === leaderboard[leaderboard.length - 1].id);
        if (updatedEntry) {
...
{#if state === 'won'}
    ...
    <div class="game-stats">
        ...
        <p>
            ...
            <button on:click={updateLeaderboard}>OK</button>
        </p>
    </div>
    ...
    <div class="leaderboard-container">
        <table>
            ...
            <tbody>
                ...
                <tr class:current-entry={entry.id === leaderboard[leaderboard.length - 1].id}>
                    <td>{index + 1}</td>
                    <td>{entry.moves}</td>
                    <td>{entry.time}</td>
                    <td>{entry.player}</td>
