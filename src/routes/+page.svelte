    ...
    function checkGameCompletion() {
        if (matchedPairs.length === gamePokemon.length) {
         ...
         // Create a new leaderboard entry
            ...
            // Update leaderboard with improved sorting
        leaderboard = [...leaderboard, newEntry]
            .sort((a, b) => {
                if (a.moves !== b.moves) {
                    return a.moves - b.moves; // Primary sort by moves
                }
                return a.time - b.time; // Secondary sort by time
            })
            .slice(0, 100);
...
function updateLeaderboard() {
    if (playerName.trim()) {
        const updatedEntry = leaderboard.find(entry => entry.time === elapsedSeconds && entry.moves === attempts);
        if (updatedEntry) {
            updatedEntry.player = playerName;
            localStorage.setItem('pokemonMatchLeaderboard', JSON.stringify(leaderboard));
...
{#if state === 'won'}
    ...
    <div class="leaderboard-container">
        <table>
            ...
            <tbody>
                ...
                <tr class:current-entry={entry.time === elapsedSeconds && entry.moves === attempts}>
                    <td>{index + 1}</td>
                    <td>{entry.moves}</td>
                    <td>{entry.time}</td>
                    <td>{entry.player}</td>
    ...
    <button class="game-button" on:click={() => { 
        state = 'start'; 
...
<style>
    ...
    .game-stats {
        margin-bottom: 2rem; /* Fügt Abstand unter den Statistiken hinzu */
    }
    ...
    .game-button {
        /* Fügen Sie hier die Eigenschaften ein, die Sie in der s-y_bCXRrkrYfP Klasse gefunden haben */
        /* Zum Beispiel: */
        border: 2px solid #3498db;
        border-radius: 4px;
        padding: 10px 20px;
        /* ... andere relevante Eigenschaften */
    }
